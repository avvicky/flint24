...
const Experiences = ({ experienceData, educationData }: Props) => {
  const [show, setShow] = useState("Registration");
  ...
  return (
    <SectionWrapper id="timeline" className="min-h-screen">
      <h2 className="text-4xl text-center">Time Line</h2>

      <div className="w-fit mx-auto mt-6 p-2 bg-white dark:bg-grey-800 rounded-md flex gap-2 items-center">
        {["Registration", "Event"].map((e, i) => (
          <button
            key={i}
            onClick={() => setShow(e)}
      ...
      <div className="lg:container sm:mx-4 lg:mx-auto lg:w-5/6 2xl:w-3/4">
        <div className="relative wrap overflow-hidden p-4 md:py-10 md:px-0">
          ...
          {viewAll
            ? (show === "Registration" ? experiences : educations).map(
                (e, i) => (
                  // @ts-ignore
                  <ExperienceCard key={i} {...e} index={i} />
                )
              )
            : (show === "Registration" ? experiences : educations)
                .slice(0, 2)
                .map((e, i) => (
                  // @ts-ignore
                  <ExperienceCard key={i} {...e} index={i} />
